...
class StratumServer:
    ...
    async def process_message(self, message: dict, writer: asyncio.StreamWriter):
        ...
        print(f"📨 Получено от {client_addr}: {method}")
    ...
    async def send_difficulty(self, writer: asyncio.StreamWriter):
        # Отправляем максимальную сложность (1)
        response = {"id": None, "method": "mining.set_difficulty", "params": [1]}
    ...
    async def send_job(self, writer: asyncio.StreamWriter):
        ...
        # Фиксированный bits для максимальной сложности
        fixed_bits = 0x1f00ffff
        ...
        block_template = Block(
            ...
            bits=fixed_bits,
            compute_hash=False  # не вычисляем хэш, т.к. майнер сам вычисляет
        )
        ...
        # Максимальный target (всегда проходим)
        ...
        print(f"⛏️ Отправлено задание {job_id} с bits={hex(fixed_bits)}")
    ...
    async def handle_submit(self, writer: asyncio.StreamWriter, msg_id, params):
        ...
        # Используем время из запроса (майнер может посылать своё)
        ntime_int = int(ntime_hex, 16) if ntime_hex != "00000000" else int(time.time())
        # Если время слишком старое или будущее, используем текущее
        ...
        print(f"🔍 Проверка: hash={block_hash_hex[:16]}..., target={target}")

        # Проверка PoW (с максимальным target всегда проходит)
        if int(block_hash_hex, 16) >= target:
            await self.send_response(writer, msg_id, False, error={"code": -1, "message": "Low difficulty"})
            return

        # Создаём блок с вычисленным хэшем
        block = Block(
